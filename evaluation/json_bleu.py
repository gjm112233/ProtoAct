...
def compute_bleu_for_value(
    ...
    """
    # Convert gt and pred parameter values into token lists.
...
def compute_bleu_for_matched_action(
    ...
    return {
        "action": gt_action,
        "param_scores": param_scores,  # Per-parameter scores for this item.
        "mean_bleu": mean_bleu
    }
...
def compute_bleu_for_all_matched_actions(
    ...
    sum_all_params_scores = sum(all_param_scores)
    ...
    return {
        ...
        "sum_all_params_scores": sum_all_params_scores,  # Sum of all parameter scores.
        "count_params": count_params  # Number of evaluated parameters.
    }


# Monitor BLEU helper functions.
# Compute BLEU for all matched monitor pairs.
def compute_bleu_for_all_matched_monitors(
    gt_matched: list[dict[str, Any]],
    pred_matched: list[dict[str, Any]],
    weights: tuple[float, float, float, float] = (0.5, 0.5, 0, 0),
) -> dict[str, Any]:
    """
    Compute parameter BLEU for all matched monitor pairs.
    ...
    all_param_scores: list[float] = []
    all_monitor_mean_scores: list[float] = []

    # Iterate through each aligned gt/pred pair.
    for gt_item, pred_item in zip(gt_matched, pred_matched):
        # Compute monitor-level parameter BLEU details.
        # Compute monitor-level parameter BLEU details.
        # Compute monitor-level parameter BLEU details.
        # Compute monitor-level parameter BLEU details.
        # Compute monitor-level parameter BLEU details.
        #     }
        result = compute_bleu_for_matched_monitor(
            gt_item,
            pred_item,
            weights=weights
        )
        ...
        all_monitor_mean_scores.append(result["mean_bleu"])
    ...
    sum_all_params_score = sum(all_param_scores)
    count_params = len(all_param_scores)
    ...
    return {
        ...
        "monitor_mean_bleu": monitor_mean_bleu,
        "sum_all_params_scores": sum_all_params_score,  # Sum of all parameter scores.
        "count_params": count_params  # Number of evaluated parameters.
    }


# Compute BLEU for all parameters in one matched monitor pair and its mean.
def compute_bleu_for_matched_monitor(
    gt_item: dict[str, Any],
    pred_item: dict[str, Any],
    weights: tuple[float, float, float, float] = (0.5, 0.5, 0, 0),
) -> dict[str, Any]:
    """
    Compute BLEU for every parameter in one matched monitor pair.
    The result records the monitor type, one BLEU score per parameter, and the mean BLEU for this monitor.
    """
...
def main() -> None:
    ...
    for cell_name in cell_names:
        ...
        # Evaluation pipeline step for the current experiment.
        # for i in range(len(gt_matched)):
        #     print("gt:", gt_matched[i], '\n', "pred", pred_matched[i])
        #
        # print(len(gt_matched), len(pred_matched))
        #
        # print("\nAction Parameter BLEU Evaluation")

        bleu_summary = compute_bleu_for_all_matched_actions(
            gt_matched,
            pred_matched,
            weights=(0.5, 0.5, 0, 0)  # 2-gram BLEU
        )

        print("global_param_mean_bleu:", round(bleu_summary["global_param_mean_bleu"], 3))
        # print("action_mean_bleu:", bleu_summary["action_mean_bleu"])
        print("action_sum_all_params_scores:", round(bleu_summary["sum_all_params_scores"], 3))
        print("action_count_params:", bleu_summary["count_params"])

        file_path = "./result/bleu_action.xlsx"
        old_df = pd.read_excel(file_path)
        # [gpt-5.3, qwen3-max, qwen-max, deepseek-v3.2, deepseek-r1, Kimi-K2, glm-4.5]
        new_row = pd.DataFrame([{
            "model_name": "qwen3-max",
            "experiment": cell_name,
            "sum_bleu": round(bleu_summary["sum_all_params_scores"], 3),
            "count_params": bleu_summary["count_params"],
        }])
        ...
        print("Monitor Parameter BLEU Evaluation")
        ...
        bleu_summary = compute_bleu_for_all_matched_monitors(
            gt_matched,
            pred_matched,
            weights=(0.5, 0.5, 0, 0)  # 2-gram BLEU
        )
        ...
        print("global_param_mean_bleu:", round(bleu_summary["global_param_mean_bleu"], 3))
        # print("action_mean_bleu:", bleu_summary["monitor_mean_bleu"])
        print("monitor_sum_all_params_scores:", round(bleu_summary["sum_all_params_scores"], 3))
        print("monitor_count_params:", bleu_summary["count_params"])

        file_path2 = "./result/bleu_monitor.xlsx"
