...
def main() -> None:
    ...
    for cell_name in cell_names:
        ...
        print("\n=== monitor_counts ===")
        print(dict(gt_monitor_counts))
        print(dict(pred_monitor_counts))

        print("\n=== subtask_counts ===")
        print(dict(gt_subtask_counts))
        print(dict(pred_subtask_counts))

        print("\n=====================================")
        # Optional diagnostic output.
        print("Action-level Precision / Recall / F1")
        result = compute_f1(gt_subtask_counts, pred_subtask_counts)

        print("TP(correct actions):", result["TP"])
        print("FP(extra predicted actions):", result["FP"])  # Number of false positives.
        print("FN(missed actions):", result["FN"])  # Number of false negatives.
        # print("precision:", result["precision"])
        # print("recall:", result["recall"])
        # print("f1:", result["f1"])

        file_path = "./result/precision_recall_action.xlsx"
        ...
        print("action append complete")

        print("\n=====================================")
        # Optional diagnostic output.
        print("Monitoring Condition Precision / Recall / F1")
        result2 = compute_f1(gt_monitor_counts, pred_monitor_counts)

        print("TP(correct monitor conditions):", result2["TP"])
        print("FP(extra predicted monitor conditions):", result2["FP"])  # Number of false positives.
        print("FN(missed monitor conditions):", result2["FN"])  # Number of false negatives.
        # print("precision:", result2["precision"])
        # print("recall:", result2["recall"])
        # print("f1:", result2["f1"])

        file_path = "./result/precision_recall_monitor.xlsx"
        old_df = pd.read_excel(file_path)
        # [gpt-5.3, qwen3-max, qwen-max, deepseek-v3.2, deepseek-r1, Kimi-K2, glm-4.5]
        new_row = pd.DataFrame([{
            "model_name": "qwen3-max",
